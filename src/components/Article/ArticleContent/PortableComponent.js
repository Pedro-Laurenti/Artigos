...
const articePortableText = {
    ...
    marks: {
      link: ({ children, value }) => {
        ...
        return (
          !value?.href?.startsWith('/') ?
            <a href={value.href} rel={rel} target={targ} className="text-appBlue-100 dark:text-appBlue-50 text-decoration-underline" aria-label={`${children}`}>
              {children}
      ...
      code: ({ children }) => <span className="font-mono leading-normal bg-opacity-15 bg-gray-300 dark:text-appBlue-100 text-appBlue-100 rounded-md text-sm px-1 py-2 font-bold"
